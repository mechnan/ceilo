function bdfcn_surface_pcolor(hObj,eventdata)
...
if strcmp(get(mainObj,'SelectionType'),'normal') % left click
    ...
    if(ishandle(handles.line_manual_pbl))
        xdata = [get(handles.line_manual_pbl,'XData'),x];
        ydata = [get(handles.line_manual_pbl,'YData'),y];
        zdata = [get(handles.line_manual_pbl,'ZData'),tag];
        cdata = [get(handles.line_manual_pbl,'CData');color_tag{tag}];
